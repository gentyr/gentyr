...
- `force_record_next_demo` — Force the next `run_demo` call to record video, regardless of staleness or agent choice. One-shot: writes a flag to `.claude/state/demo-force-record.json` which is consumed and deleted by the next `run_demo` invocation. Accessible via the `/force-record` slash command.
