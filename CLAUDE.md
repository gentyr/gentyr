...
- `preflight_check` — Validate environment readiness before launching; runs 9 checks: config exists, dependencies, browsers installed, test files, credentials valid, dev server reachable (check #6 — now **fails** rather than warns when unreachable or returns app-level errors; uses GET with body inspection for error patterns like "Application error" or "Internal Server Error"), compilation, auth state freshness, and extension manifest valid
...
- Event types: `suite_begin` (run starts, total test count), `test_begin` (individual test starts), `step` (pw:api and expect steps only — noise filtered), `test_end` (test finishes with status, duration, and error on failure), `console_error` (error pattern detected in stderr — does NOT set `has_failures`), `suite_end` (all tests finish with aggregates)
...
- `recent_errors` accumulates up to 10 entries from `console_error` events and `crash` events (written directly by `server.ts` on startup crash); used for informational display only in `check_demo_result`
...
- `/demo` calls `mcp__playwright__launch_ui_mode`; `/demo-interactive`, `/demo-autonomous`, and `/demo-all` call `mcp__playwright__run_demo` with `test_file` and `pause_at_end` from the selected scenario
