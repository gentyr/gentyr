...
**Page 5 — Feed**: Live AI commentary feed. Every 60 seconds, `getActivityFingerprint()` checks for DB mtime changes across `session-queue.db`, `session-activity.db`, and `plans.db`. When activity is detected, a Haiku-tier `claude -p` subprocess is spawned with streaming JSON output to narrate what is currently happening across all running agents. Commentary is built from a `CommentaryContext` assembled by `readCommentaryContext()` (running sessions with last tool/message, recent session summaries, active plan progress) and includes the last 10 messages for continuity. Streaming text appears with a block-cursor indicator as Haiku generates; the completed message is added to the feed on the `result` event. Feed supports scroll (up/down arrows), page scroll (pgUp/pgDn), and auto-follow mode (re-engages on new content when on the page; `end` key to manually re-engage). Key modules: `hooks/useLiveFeed.ts` (hook), `components/CommentaryView.tsx` (view), `live-reader.ts` exports `readCommentaryContext()` and `getActivityFingerprint()`. Mock data: `getMockPage5Data()` in `mock-data.ts`.
