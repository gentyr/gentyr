...
- Chrome-bridge scenarios and extension demos always run locally. Scenarios with `remote_eligible=false` in the DB are also forced local — this is the authoritative override that takes priority over all heuristic checks. `DEMO_HEADLESS` is set dynamically from the resolved `headless` flag (`'0'` when headed, `'1'` otherwise).
