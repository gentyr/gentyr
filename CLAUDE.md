...
Foundation for a single source of truth for cross-entity task dependencies across **todo**, **persistent**, and **plan_task** entities. The `workstream.queue_dependencies` table is extended with `blocked_entity_type` / `blocker_entity_type` (`'todo' | 'persistent' | 'plan_task'`, default `'todo'`) plus a `pause_action` column (`'killed_session' | 'paused_persistent' | 'paused_plan_task' | NULL`). The `UNIQUE` constraint is broadened to `(blocked_entity_type, blocked_task_id, blocker_entity_type, blocker_task_id)` via shadow-table swap (SQLite cannot ALTER a UNIQUE), and the narrow indexes are replaced with composite `(entity_type, task_id, status)` indexes. Migration is idempotent on every DB open and preserves existing rows.

**`add_dependency` (backward-compatible union)**: Accepts both new shape `{ blocker: {entity_type, entity_id}, blocked: {entity_type, entity_id}, reasoning }` and legacy `{ blocker_task_id, blocked_task_id, reasoning }` (normalized to todo→todo). Validates both entities exist in their source DB (`todo.db`, `persistent-tasks.db`, or `plans.db`). Entity-aware cycle detection (DFS over `(entity_type, entity_id)` nodes). Already-completed short-circuit: inserts the dep as `'satisfied'` when the blocker is in a satisfying terminal state. **Pause-if-running** on the blocked entity (CTO-confirmed semantics — kill + reset / pause):
...
**`list_dependencies_for_entity` (new tool)**: Query deps involving a specific entity with `direction='blocking' | 'blocked_by' | 'both'` (default `both`). Used by plan-managers and persistent-monitors to surface why a child is not spawning.

**Backward compatibility**: `areDependenciesMet()` and `satisfyCompletedDeps()` are deliberately restricted to `entity_type='todo'` so the existing `session-queue.js` gate sees no behavior change. Three gate extensions (session-queue todo gate, `activate_persistent_task` refusal, plan-orchestrator `areDependenciesMet`), three creator-tool extensions (`depends_on` on `create_task` / `create_persistent_task` / `add_plan_task`), the shared satisfier module `.claude/hooks/lib/cross-dep-satisfier.js`, and doc updates ship in follow-up PRs.
...
| agent-tracker | get_session_queue_status, set_max_concurrent_sessions, acquire/release_shared_resource, submit/resolve_bypass_request, list/resolve_blocking_item, get_blocking_summary, peek_session, browse_session, set_automation_toggle, get_automation_toggles, record_cto_decision, check_cto_decision, cto_decision_audit_pass, cto_decision_audit_fail, query_token_usage, top_token_sessions, token_attribution_health, revival_cost_summary | Session queue, signals, locks, bypass, blocking queue, automation toggles, CTO authorization chain, token usage attribution |
