...
- Headed demos default to local (requires a physical display), but can be forced remote by passing `remote: true` explicitly — the Fly.io machine handles display and recording via Xvfb + ffmpeg (`DEMO_HEADLESS=0`). Chrome-bridge scenarios and extension demos always run locally. `DEMO_HEADLESS` is set dynamically from the request's `headless` flag (`'0'` when `headless === false`, `'1'` otherwise).
