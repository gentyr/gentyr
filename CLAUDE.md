...
**Active session protection**: `cleanupMergedWorktrees()` in `worktree-manager.js` checks `isWorktreeInUse()` (`lsof +D`) before removing merged worktrees to protect live sessions from CWD eviction, and also cross-checks `session-queue.db` for any `running/queued/spawning/suspended` sessions using the worktree path (PR #475 extended this from `suspended`-only to all active statuses). `isWorktreeInUse()` is **fail-closed** (PR #475): it returns `true` (assume in use) on any `lsof` error or timeout — only returns `false` when `lsof` exits with code 1 and empty stdout (confirmed no processes). Previously it returned `false` on any error (fail-open). `removeWorktree()` (called by stale reaper path and reactive cleanup) uses `killProcessesInDirectory()` to kill all processes with open files in the worktree before attempting removal; the stale reaper and rescue paths now skip rather than kill when active processes are found. The `worktree-cwd-guard.js` hook additionally detects stale CWD at tool-call time and blocks Bash execution with a recovery hint if the worktree directory no longer exists.
