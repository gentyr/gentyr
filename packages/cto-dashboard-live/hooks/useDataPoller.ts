...
export function useDataPoller(intervalMs: number, mock: boolean): DataPollerResult {
  ...
  const completedOffset = useRef(20); // initial batch is 20
  const loadingMore = useRef(false);
  const hasMoreRef = useRef(true);

  // Periodic poll — refreshes everything except extra loaded completed
  useEffect(() => {
    if (mock) return;
    const id = setInterval(() => {
      try {
        const fresh = readLiveData();
        // Merge extra completed sessions loaded via infinite scroll
  ...
  const loadMoreCompleted = useCallback(() => {
    ...
    try {
      ...
      if (more.length === 0) {
        ...
        // Immediately merge into current data
        setData(prev => ({
          ...prev,
