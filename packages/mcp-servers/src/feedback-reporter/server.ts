...
const FEEDBACK_PERSONA_NAME = process.env['FEEDBACK_PERSONA_NAME'];
const FEEDBACK_SESSION_ID = process.env['FEEDBACK_SESSION_ID'];

// Validation: Required environment variables
if (!FEEDBACK_PERSONA_NAME) {
...
const SESSIONS_DIR = path.join(PROJECT_DIR, '.claude', 'feedback-sessions');
const SESSION_DB_PATH = path.join(SESSIONS_DIR, `${FEEDBACK_SESSION_ID}.db`);
const REPORTS_DB_PATH = path.join(PROJECT_DIR, '.claude', 'cto-reports.db');
...
function initializeReportsDb(): Database.Database {
  const dbDir = path.dirname(REPORTS_DB_PATH);
  if (!fs.existsSync(dbDir)) {
    fs.mkdirSync(dbDir, { recursive: true });
  }

  const db = new Database(REPORTS_DB_PATH);
  db.pragma('journal_mode = WAL');
...
function getSessionDb(): Database.Database {
  if (!_sessionDb) {
    _sessionDb = initializeSessionDb();
  }
  return _sessionDb;
}
...
function getReportsDb(): Database.Database {
  if (!_reportsDb) {
    _reportsDb = initializeReportsDb();
  }
  return _reportsDb;
}
...
function submitFinding(args: SubmitFindingArgs): SubmitFindingResult | ErrorResult {
  ...
  try {
    ...
    const reportingAgent = `feedback-${FEEDBACK_PERSONA_NAME}`;
...
function submitSummary(args: SubmitSummaryArgs): SubmitSummaryResult | ErrorResult {
  ...
  try {
    ...
    // 2. Build summary for agent-reports
    const title = `Feedback Summary: ${FEEDBACK_PERSONA_NAME} - ${args.overall_impression}`;
    ...
    summary += `\n\nSession ID: ${FEEDBACK_SESSION_ID}`;
