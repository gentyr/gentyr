...
/**
 * Override the icons directory. Restricted to test environments to prevent
 * concurrent tool calls from corrupting each other's state in production.
 */
export function setIconsDir(dir: string): void {
  if (process.env.NODE_ENV !== 'test' && typeof process.env.VITEST === 'undefined') {
    throw new Error('setIconsDir is only available in test environments');
  }
  ICONS_DIR = dir;
}
...
/** RFC 1918 / loopback / link-local / cloud metadata IP ranges to block (SSRF prevention) */
...
function isBlockedIp(hostname: string): boolean {
  ...
  //   hex:     ::ffff:7f00:1  (URL parser normalizes to this form)
  const v4MappedDotted = lower.match(/^::ffff:(\d+\.\d+\.\d+\.\d+)$/);
  if (v4MappedDotted) {
    const mapped = v4MappedDotted[1].split('.').map(Number);
    if (mapped.length === 4 && mapped.every((n) => !isNaN(n) && n >= 0 && n <= 255)) {
...
/**
 * Validate a URL is http:// or https:// only and does not target private/internal networks.
 * Blocks file://, ftp://, data:// protocols and RFC 1918, loopback, link-local, and cloud metadata IPs.
 */
function assertSafeUrl(url: string): void {
  ...
  if (BLOCKED_HOSTNAMES.has(hostname) || isBlockedIp(hostname)) {
    throw new Error(`Blocked URL target: ${hostname} — requests to private/internal networks are not allowed`);
  }
}
...
/**
 * Resolve a path through symlinks. For non-existent paths, resolves the
 * nearest existing ancestor and appends the remaining segments.
 * This handles macOS `/var/folders/` → `/private/var/folders/` correctly.
 */
function safeRealpath(p: string): string {
  try {
    return fs.realpathSync(p);
  } catch {
    // Path doesn't exist — resolve the parent, append the basename
...
/**
 * Verify a path is contained within the expected parent directory.
 * Resolves symlinks to prevent symlink escapes.
 */
function assertContainedIn(filePath: string, parentDir: string): void {
  const resolved = safeRealpath(filePath);
  const resolvedParent = safeRealpath(parentDir);
  if (!resolved.startsWith(resolvedParent + path.sep) && resolved !== resolvedParent) {
    throw new Error(`Path escapes expected directory: ${filePath} is not within ${parentDir}`);
  }
}
...
async function downloadImage(args: DownloadImageArgs): Promise<DownloadImageResult> {
  ...
  // Streaming download with size accumulator — abort before OOM if body exceeds limit
  const chunks: Buffer[] = [];
  let totalBytes = 0;
    ...
    for await (const chunk of body) {
      ...
      if (totalBytes > MAX_DOWNLOAD_BYTES) {
        ...
        return {
          ...
          error: `Downloaded content too large: exceeded ${MAX_DOWNLOAD_BYTES} byte limit during streaming`,
        };
      }
      chunks.push(buf);
    }
...
async function listIcons(_args: ListIconsArgs): Promise<ListIconsResult> {
  ...
  for (const entry of entries) {
    ...
    assertContainedIn(brandDir, ICONS_DIR);
