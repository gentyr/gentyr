...
// ============================================================================
// (Lockdown / Local Mode tool implementations removed)
// ============================================================================
...
async function stageMcpServer(args: StageMcpServerArgs): Promise<object> {
  ...
  try {
    ...
    return {
      ...
      pending: true,
      server_name: args.name,
      message: `Server "${args.name}" staged (project is protected). Ask the CTO to run: sudo true && npx gentyr sync, then restart the Claude Code session for new MCP tools to appear.${collisionWarning}`,
    };
