...
function getBypassDb(): InstanceType<typeof Database> {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS cto_decisions (
      ...
      consumed_at TEXT,
      created_at TEXT NOT NULL DEFAULT (datetime('now')),
      CHECK (decision_type IN ('bypass_request', 'protected_action', 'lockdown_toggle', 'release_signoff', 'staging_override', 'deputy_bypass_resolution', 'deputy_deferred_approval')),
  ...
  // Migration: expand cto_decisions CHECK constraint to include deputy decision types
  ...
  try {
    ...
    try {
      ...
      if (checkErr instanceof Error && checkErr.message.includes('CHECK constraint')) {
        // The old table may not have decision_context column. We must handle the column mismatch.
        // First add decision_context to the old table if missing (so INSERT INTO ... SELECT * works).
        try {
          const oldCols = db.prepare("PRAGMA table_info(cto_decisions)").all() as Array<{ name: string }>;
          if (!oldCols.some((c: { name: string }) => c.name === 'decision_context')) {
        ...
        db.exec(`
          ...
          INSERT INTO cto_decisions_new SELECT * FROM cto_decisions;
  ...
  // Migration: add decision_context column to cto_decisions if absent
  try {
    const decCols = db.prepare("PRAGMA table_info(cto_decisions)").all() as Array<{ name: string }>;
    if (!decCols.some((c: { name: string }) => c.name === 'decision_context')) {
      db.exec("ALTER TABLE cto_decisions ADD COLUMN decision_context TEXT");
    }
  } catch { /* non-fatal migration */ }
...
const server = new McpServer({
  name: 'agent-tracker',
  version: '9.7.0',  // Phase 4: Route deputy resolutions through auditor (deputy_bypass_resolution, deputy_deferred_approval decision types)
  tools,
});
