...
const tools: AnyToolHandler[] = [
  {
    ...
    description: 'Report an issue to the deputy-cto for triage. Deputy-cto reviews and may escalate to CTO queue. Use for architecture decisions, security concerns, blockers, or important changes. Idempotent: if a pending report with the same idempotency_key (or same reporting_agent + title when no key is provided) already exists, the existing report ID is returned without creating a duplicate.',
