...
/**
 * Local prototyping mode has been removed from GENTYR.
 *
 * This function is retained as a permanent stub that ALWAYS returns false so
 * the (~14) callers across the codebase continue to import and run cleanly,
 * always taking the production (non-local) code path. Do not reintroduce any
 * local-mode.json read here.
 *
 * @param {string} [_projectDir] - Ignored.
 * @returns {boolean} Always false.
 */
export function isLocalModeEnabled(_projectDir) {
  return false;
}
