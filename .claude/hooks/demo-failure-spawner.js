    ...
    enqueueSession({
      ...
      buildPrompt: (agentId) => {
        ...
        return [
          ...
          `## Instructions`,
          ``,
          `**MANDATORY: Use Fly.io remote execution (the default).** Run demos via \`run_demo\` with no routing flags — by default this routes to Fly.io with \`recorded: true\`. Remote execution avoids display lock contention and produces identical video recordings via Xvfb+ffmpeg. NEVER pass \`local: true\` (it is CTO-gated and blocks spawned agents); local-only scenarios (chrome-bridge, \`remote_eligible=false\`) are routed locally by the server automatically. Do NOT run demos locally "to debug" — remote execution produces the same output with better diagnostics. If you need to re-run multiple scenarios, use \`run_demo_batch({ scenario_ids: [...], recorded: true })\` for concurrent Fly.io execution across multiple machines.`,
          ``,
          ...
          `7. Re-run the scenario remotely to verify (\`run_demo\` with defaults — routes to Fly.io with recording)`,
