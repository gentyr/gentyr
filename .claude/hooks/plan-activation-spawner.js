...
async function main() {
  ...
  try {
    ...
    if (result.blocked) {
      ...
      const ctx = `[PLAN ACTIVATION] Plan "${planTitle}" activated. Persistent task ${ptId} created but monitor enqueue was blocked (${result.blocked}). The plan-manager will start when the block clears.`;
    ...
    const successCtx = `[PLAN ACTIVATION] Plan "${planTitle}" activated with plan-manager persistent task ${ptId}. Monitor enqueued (queueId: ${result.queueId}). The plan-manager will automatically advance phases as they complete.`;
    process.stdout.write(JSON.stringify({ additionalContext: successCtx }));
  } catch (err) {
    log(`Error enqueuing plan-manager: ${err.message}`);
    const errCtx = `[PLAN ACTIVATION] Persistent task ${ptId} created for plan "${planTitle}" but failed to enqueue monitor: ${err.message}. Resume manually with: mcp__persistent-task__resume_persistent_task({ id: "${ptId}" })`;
