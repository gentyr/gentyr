...
async function main() {
  ...
  // When lockdown is disabled, the CTO is babysitting the 6-step pipeline directly
  // in-session. ALL agent types pass through — including code-writer/test-writer/
  // code-reviewer/demo-manager. The CTO drives the sequence interactively and watches
  // each step land in the per-session cto-interactive-<sid8> worktree.
  try {
