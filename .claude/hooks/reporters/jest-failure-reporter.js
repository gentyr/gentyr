 ...
 * - [Automation][test-failure-jest] prefix for CTO dashboard tracking
...
async function getConfiguredCooldown() {
  try {
    ...
    console.error('[jest-failure-reporter] Warning:', err.message);
    return 120;
  }
}
...
async function spawnClaude(suiteNames, failureDetails) {
  ...
  // Use [Automation][test-failure-jest] format for CTO dashboard tracking
  const prompt = `[Automation][test-failure-jest] ${promptTemplate}
...
\`\`\``;
  ...
  try {
    ...
    enqueueSession({
      title: `Fixing ${suiteNames.length} failing Jest test suite(s): ${suiteNames.slice(0, 3).join(', ')}`,
      agentType: AGENT_TYPES.TEST_FAILURE_JEST,
      hookType: HOOK_TYPES.JEST_REPORTER,
      tagContext: 'test-failure-jest',
      source: 'jest-failure-reporter',
      prompt,
      priority: 'normal',
      cwd: projectRoot,
