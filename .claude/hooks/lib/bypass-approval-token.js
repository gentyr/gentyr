/**
 * Shared module: HMAC-verified bypass approval token verification.
 *
 * Centralizes the logic for reading, verifying, and consuming the
 * `.claude/bypass-approval-token.json` written by `bypass-approval-hook.js`
 * after the CTO types `APPROVE BYPASS <code>` in chat.
 *
 * Security model:
 *  - The 6-char code is server-generated by `mcp__deputy-cto__request_bypass`
 *    and stored in deputy-cto.db. Agents cannot forge it.
 *  - The HMAC is signed with `.claude/protection-key`. Agents cannot forge it.
 *  - The token is one-time-use: consumed (overwritten with `{}`) on success.
 *  - Expiry is 5 minutes from issuance.
 *
 * G001 fail-closed: missing protection-key → invalid (not allowed).
 *
 * TODO(future): `block-no-verify.js` `hasValidBypassToken()` and
 * `deputy-cto/server.ts` `execute_bypass` replicate this logic. Migrate
 * those callers to use this module in a future PR to consolidate.
 *
