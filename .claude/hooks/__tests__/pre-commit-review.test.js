...
// =============================================================================
// G020: Branch-Aware Commit Blocking - REMOVED
//
// G020 commit blocking was removed in Phase 2 of the production promotion
// overhaul. Production releases are now CTO-initiated via /promote-to-prod.
// The hasPendingCtoItems() function is kept for session briefing use only.
//
// Anti-regression test below verifies the blocking code stays removed.
// =============================================================================

describe('G020: Branch-Aware Commit Blocking - REMOVED (Anti-Regression)', () => {
  const HOOK_PATH = path.join(process.cwd(), '.claude/hooks/pre-commit-review.js');

  it('should NOT contain G020 branch-aware commit blocking code', () => {
    const hookCode = fs.readFileSync(HOOK_PATH, 'utf8');
    // G020 was removed in Phase 2 of the production promotion overhaul.
    // These patterns must NOT exist in the production code.
    assert.doesNotMatch(hookCode, /COMMIT BLOCKED: Pending CTO item/,
      'G020 commit blocking must be removed — production releases use /promote-to-prod');
  });

  it('should still have hasPendingCtoItems() for session briefing use', () => {
    const hookCode = fs.readFileSync(HOOK_PATH, 'utf8');
    assert.match(hookCode, /function hasPendingCtoItems\(\)/,
      'hasPendingCtoItems() must be kept for session briefing');
  });

  it('should still have protected branch guard (separate from G020)', () => {
    const hookCode = fs.readFileSync(HOOK_PATH, 'utf8');
    assert.match(hookCode, /PROTECTED_BRANCHES/,
      'Protected branch guard must still exist');
  });
});


// G020 logic and code structure tests removed — see anti-regression block above.
