...
6. Search for any recent icon/logo redesigns: `"[brand] icon redesign"` or `"[brand] new logo [current year]"` — note the expected current icon appearance for use in Phase 2.5 validation.
...
### Phase 2.5 — Candidate Analysis & Validation

Before processing candidates, analyze them to ensure you have the RIGHT icon — not just any icon.

**Step 1: Describe each candidate**

For each file in `~/.claude/icons/<slug>/artifacts/candidates/`:
- If SVG: call `mcp__icon-processor__analyze_svg_structure` for element counts, colors, and shape types. Then `Read` the file to visually inspect it.
- If raster (PNG/WEBP/ICO): call `mcp__icon-processor__analyze_image` for dimensions, dominant colors, and background type. Then `Read` the file to visually inspect it.
- Write a concise description of the icon's **concept** — what shape/design does it depict? (e.g., "Modern gradient swoosh/wave", "Flat geometric 'A' lettermark", "Circular emblem with shield motif"). Focus on the fundamental design, not color or size.

**Step 2: Identify distinct icon concepts**

Compare your descriptions. Group candidates that are style variants of the same underlying design (same shape, different colors/gradients/detail levels). Flag candidates that represent **fundamentally different icons** — different shapes, different design eras, or different brand marks entirely.

If all candidates are variants of the same concept → skip to Phase 3 (no validation needed).

**Step 3: Research current official icon**

When multiple distinct concepts are detected:
1. `WebSearch` for `"[brand] current icon [current year]"`, `"[brand] logo redesign"`, `"[brand] new brand identity"`
2. `WebSearch` for `"[brand] icon"` on the brand's official site (add `site:brand.com` if known)
3. Determine: What does the brand's **current** icon look like? Has there been a recent redesign? What is the expected shape?
4. Summarize: "The current [brand] icon is [description]. It was [introduced/redesigned] in [year]."

**Step 4: Validate and prune**

Compare each candidate against the current icon description:
- **Valid**: Matches the current official icon design (may differ in color/detail, but same fundamental shape)
- **Rejected**: Represents an outdated, deprecated, or wrong icon design

Actions:
- Delete rejected candidates from `artifacts/candidates/` (they would waste processing time)
- If **no valid candidates remain**: search specifically for the current icon design, download new candidates to `artifacts/candidates/`, and **restart Phase 2.5** (max 2 retries — if still no match after 2 extra rounds, proceed with the best available)

**Step 5: Document analysis**

Write `~/.claude/icons/<slug>/artifacts/candidate-analysis.md` with:
- Per-candidate descriptions and concept groupings
- Research findings on current icon (with sources)
- Which candidates were kept vs rejected (with reasons)
- Any additional search rounds performed

This analysis feeds into the Phase 6 report.
...
5. Write the report content (passed to `store_icon` above as `report_md`) documenting:
   ...
   - Reference the candidate analysis from Phase 2.5 (concept groupings, icon history research, pruning decisions)
...
~/.claude/icons/<slug>/
  artifacts/
    ...
    candidate-analysis.md ← Phase 2.5 validation report
