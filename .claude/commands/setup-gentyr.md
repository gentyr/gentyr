...
```bash
GENTYR_DIR="$([ -d node_modules/gentyr ] && echo node_modules/gentyr || { [ -d .claude-framework ] && echo .claude-framework || echo .; })" && echo "$GENTYR_DIR"
```

Use this `$GENTYR_DIR` value for all subsequent `node` commands and `Read` tool paths in this flow.
...
```bash
node "$GENTYR_DIR/scripts/setup-check.js" 2>/dev/null
```
...
1. If `gentyrInstalled === false`:
   ...
   - Provide: `sudo "$GENTYR_DIR/scripts/reinstall.sh" --path $(pwd) --op-token <token>`
...
2. **Read** `$GENTYR_DIR/docs/SETUP-GUIDE.md`.
...
1. **Read** `$GENTYR_DIR/docs/SETUP-GUIDE.md`.
...
2. **Run permission validation** (always — do NOT offer this as optional):
   ```bash
   node "$GENTYR_DIR/scripts/setup-validate.js" 2>/dev/null
   ```
...
4. **For any missing credentials** (`existsInOp === false` OR `mappedInVault === false`):
   - Read `$GENTYR_DIR/docs/SETUP-GUIDE.md`
...
5. **For any failed validations** (status: `fail`):
   ...
   - Also read and output the relevant `$GENTYR_DIR/docs/SETUP-GUIDE.md` section for context
...
2. **Install merge-chain-check workflow**:
   ...
   - If missing, copy from GENTYR template: `$GENTYR_DIR/templates/config/merge-chain-check.yml.template`
...
4. **Branch protection setup**:
   ...
   - Display the branch protection instructions from `$GENTYR_DIR/docs/DEPLOYMENT-FLOW.md` (Branch Protection Setup section)
